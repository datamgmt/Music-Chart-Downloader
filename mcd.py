...
def date_validator(check_date, chart):
    ...
    last_chart_date = today - datetime.timedelta( (7 - chart_day + today.weekday()) % 7)
    ...
    check_date = check_date - datetime.timedelta( (7 - chart_day + check_date.weekday()) % 7)
    ...
    for folder in folder_list:
        if not os.path.exists(f"{arglist.datadir}/{folder}"):
            try:
                ...
                sys.exit(f"Failed to make directory {arglist.datadir}/{folder}")
...
parser.add_argument("--output_set",
    nargs="*",
    choices=["weekly", "all"],
    default=["all"],
